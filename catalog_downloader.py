...
from pandas_estat import set_appid, read_statslist
...
class EStatCatalogDownloader:
    ...
    def download_stats_list_by_field(self, field_code: str, limit: int = 1000) -> pd.DataFrame:
        ...
        Returns:
            統計表リストのDataFrame
        """
        print(f"統計分野 {field_code} ({self.MAJOR_FIELDS.get(field_code, '不明')}) のデータを取得中...")
    ...
    def classify_by_field(self, catalog: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        for field_code, keywords in field_keywords.items():
            ...
            if not field_catalog.empty:
                ...
                print(f"分野 {field_code} ({self.MAJOR_FIELDS[field_code]}): {len(field_catalog)}件")
        
        # 分類されなかったものは「その他」に
        classified_stats = pd.concat(classified_catalogs.values()) if classified_catalogs else pd.DataFrame()
        ...
        summary = {
            ...
            "field_summary": {
                field_code: {
                    ...
                    "organizations": catalog['GOV_ORG'].unique().tolist() if 'GOV_ORG' in catalog.columns else [],
                    "survey_years": sorted(catalog['SURVEY_DATE'].dropna().unique().tolist()) if 'SURVEY_DATE' in catalog.columns else []
                }
                for field_code, catalog in catalogs.items()
            }
        }
    ...
    def create_catalog_index(self, catalogs: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        for field_code, catalog in catalogs.items():
            # 統計調査の種類を集計
            survey_types = catalog.groupby(['STAT_NAME', 'GOV_ORG']).size().reset_index(name='table_count')
            ...
            for _, row in survey_types.iterrows():
                index_data.append({
                    ...
                    ]['SURVEY_DATE'].max() if 'SURVEY_DATE' in catalog.columns else None
                })
