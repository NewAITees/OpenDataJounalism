...
{{"stat_id": "統計ID", "theme": "記事のテーマ（20字以内）", "angle": "切り口・問い（30字以内）"}}"""
    ...
    try:
        resp = requests.post(
            ...
            json={"model": OLLAMA_MODEL, "prompt": prompt, "stream": False,
                  "think": False, "options": {"temperature": 0.9, "num_predict": 200}},
            timeout=120,
        )
    ...
    return {"stat_id": fallback_id, "theme": STAT_CATALOG[fallback_id][:20], "angle": "地域格差を探る"}
...
def _sample_data(topic: dict) -> pd.DataFrame:
    ...
    # 汎用サンプル（都道府県×数値2列）
    base = pd.DataFrame({
        "prefecture": ["東京都", "大阪府", "愛知県", "神奈川県", "福岡県",
                        "北海道", "宮城県", "広島県", "新潟県", "沖縄県"],
