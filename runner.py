...
設定:
  config.yaml でモデル・パス・タイムアウト等を変更できます。
"""
...
# --- Ollama 設定 ---
OLLAMA_URL: str = _CFG["ollama"]["url"]
MODEL: str = _CFG["ollama"]["model"]
OLLAMA_TIMEOUT: int = _CFG["ollama"]["timeout"]
OLLAMA_OPTIONS: dict[str, Any] = _CFG["ollama"]["options"]

# --- パイプライン設定 ---
PIPELINE_TIMEOUT: int = _CFG["pipeline"]["timeout"]

# --- パス設定 ---
PIPELINE = PROJECT / "pipeline.py"
PROGRAM_MD = PROJECT / _CFG["paths"]["program_md"]
LESSONS_MD = PROJECT / _CFG["paths"]["lessons_md"]
OUTPUT_DIR = PROJECT / _CFG["paths"]["output_dir"]
RUNS_DIR = PROJECT / _CFG["paths"]["runs_dir"]
BEST_PIPELINE = PROJECT / _CFG["paths"]["best_pipeline"]
...
def _ollama(prompt: str, max_tokens: int | None = None) -> str:
...
def _extract_valid_stat_ids() -> dict[str, str]:
    """pipeline.py の STAT_CATALOG から実在確認済みIDを抽出する。"""
    try:
        code = PIPELINE.read_text(encoding="utf-8")
        # "0000000000": "説明文" の形式を正規表現で抽出
        pairs = re.findall(r'"(\d{10})"\s*:\s*"([^"]+)"', code)
...
def evaluate(story: str, run_dir: Path) -> dict[str, Any]:
    ...
    valid_ids = _extract_valid_stat_ids()
    valid_ids_text = "\n".join(f"  {sid}: {desc}" for sid, desc in valid_ids.items())

    prompt = f"""あなたはデータジャーナリズムの編集長です。
以下の記事を評価してください。回答はJSONのみ（他のテキスト一切不要）。

【重要: e-Stat API で実在を確認済みの統計ID一覧】
以下のIDはすべて e-Stat API（STATUS=0）で実在が確認されています。
記事中にこれらのIDが登場する場合、「架空」「存在しない」と判定してはなりません。
{valid_ids_text}
...
def validate(code: str) -> bool:
    ...
    if '"think": False' in code or "'think': False" in code:
        ...
        if options_idx != -1 and think_idx > options_idx:
            print("  [WARN] think=False が options の中に入っています")
            return False
    # STAT_CATALOG に無効IDが混入していないか確認
    valid_ids = set(_extract_valid_stat_ids().keys())
    found_ids = set(re.findall(r'"(\d{10})"', code))
    invalid = found_ids - valid_ids
    if invalid:
        print(f"  [WARN] 無効な統計IDが混入しています: {invalid}")
        return False
    return True
...
5. 【STAT_CATALOG 厳守】以下のIDのみ使用可。これ以外のIDをコードに追加してはならない:
{chr(10).join(f"   {sid}: {desc}" for sid, desc in _extract_valid_stat_ids().items())}
6. generate_story() のプロンプトには「以下の実数値のみ引用すること。この数値以外を記事に書いてはならない」と明記すること
7. generate_story() に再帰呼び出し（自分自身を呼ぶ）を書いてはならない
...
def main() -> None:
    ...
    try:
        while args.cycles == 0 or cycle < args.cycles:
            ...
            if score > best_score:
                ...
                print(f"  [BEST] ベスト更新! {best_score}/100 → pipeline_best.py に保存")
    ...
    print(f"  ベストpipeline: {BEST_PIPELINE.relative_to(PROJECT)}")
    print(f"  ベスト記事    : {(OUTPUT_DIR / 'story_best.md').relative_to(PROJECT)}")
    print("=" * 60)


def _save_meta(run_dir: Path, cycle: int, elapsed: float, score: int, success: bool) -> None:
    meta = {
        "cycle": cycle,
